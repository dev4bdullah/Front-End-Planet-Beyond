...
/* ---------- 7. linking config vs registered screens ----------
   A deep-link config whose route names don't match the navigator tree fails
   SILENTLY — the link opens the app's default screen instead of erroring. So
   collect every <X.Screen name="…"> in the project and every route name in
   linking.js, and make sure the second set is a subset of the first. */
...
for (const [file, ast] of astCache) {
  ...
  traverse(ast, {
    JSXOpeningElement(nodePath) {
      ...
      const nameAttribute = nodePath.node.attributes.find(
        attribute => attribute.name?.name === "name"
      );
      if (nameAttribute?.value?.type === "StringLiteral") {
        screenNames.add(nameAttribute.value.value);
      }
    }
  });
}

const linkingFile = path.join(SRC, "navigation", "linking.js");

if (existsSync(linkingFile)) {
  const linkingAst = astCache.get(linkingFile) ?? parseFile(linkingFile);
  const referenced = new Set();

  traverse(linkingAst, {
    ObjectProperty(nodePath) {
      // Keys under a `screens` object are route names
      const parentProperty = nodePath.findParent(
        item => item.isObjectProperty() && item.node.key?.name === "screens"
      );
      if (!parentProperty) return;

      const key = nodePath.node.key?.name ?? nodePath.node.key?.value;
      if (key && key !== "screens") referenced.add(key);
    }
  });

  stats.linkRoutes = referenced.size;

  referenced.forEach(name => {
    // "NotFound" style catch-alls and nested `screens` wrappers are fine
    if (!screenNames.has(name)) {
      problems.push(
        `src/navigation/linking.js — route "${name}" has no matching <X.Screen name="${name}">; ` +
          `the deep link would silently open the default screen`
      );
    }
  });
}
...
console.log(
  `matched       ${stats.linkRoutes ?? 0} deep-link routes against ${screenNames.size} screens\n`
);
